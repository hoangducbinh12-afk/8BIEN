...
# --- 1. CONFIG MOBILE SIÊU NHỎ ---
st.set_page_config(page_title="8-BIT MASTER V2.6", layout="centered")
...
st.markdown("""
    ...
    .dan-box { 
        ...
        padding: 6px; margin-bottom: 8px; font-family: monospace; 
...
# --- 3. CƠ CHẾ KHỞI TẠO DỮ LIỆU (QUAN TRỌNG) ---
...
with st.expander("📝 CẬP NHẬT KẾT QUẢ", expanded=True):
    ...
    if st.button("🚀 LƯU NHẬT KÝ & NHẢY KỲ"):
        if len(num_in) >= 2:
            target = int(num_in[-2:])
            # Tính rank dựa trên số cũ nhất TRƯỚC khi cập nhật
            rank_val = get_rank_v26(target, st.session_state.last_n)
...
# --- 5. HIỂN THỊ DÀN VÀ NHẬT KÝ (FIX NAMEERROR) ---
# Khởi tạo tabs ở ngoài mọi khối lệnh if để đảm bảo biến tab1, tab2 luôn tồn tại
...
with tab2:
    if st.session_state.history:
        ...
        for h in st.session_state.history[:50]:
            ...
            b = encode_8bit(s_val)
            disp.append({
                "Kỳ": int(h.get("Kỳ", 0)), "Số": s_val, "R": int(h.get("Rank", 0)),
...
with st.sidebar:
    ...
    st.download_button("💾 XUẤT BACKUP JSON", json.dumps(backup), "8bit_master_v2.6.json", use_container_width=True)
