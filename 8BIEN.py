...
# --- 1. GIAO DIỆN CHUẨN V7.4 ---
st.set_page_config(page_title="8-BIT QUANTUM V7.4", layout="wide")
st.markdown("""
    <style>
...
def analyze_v74(history, last_n):
...
with st.sidebar:
    ...
    if up is not None and up != st.session_state.last_uploaded_file:
        data = json.load(up)
        ...
        st.session_state.last_uploaded_file = up
        st.rerun()

    if st.button("🔴 RESET"):
        st.session_state.history = []; st.session_state.last_n = -1; st.session_state.last_uploaded_file = None; st.rerun()
    
    if st.session_state.history:
        st.download_button("💾 XUẤT BACKUP", json.dumps({"history": st.session_state.history}), f"8bit_v7.4_backup.json")

# --- 5. NHẬP LIỆU ---
st.title("🛡️ 8-BIT QUANTUM V7.4")
with st.container():
    c1, c2, c3, c4 = st.columns([1.5, 1, 1.5, 2])
    n_in = c1.text_input("Số vừa nổ:", placeholder="75", key="in_so")
    def_ky = st.session_state.history[-1]["Kỳ"] + 1 if st.session_state.history else 1
    ky_in = c2.number_input("Kỳ:", value=def_ky, step=1, key="in_ky")
    
    if c3.button("🚀 PHÂN TÍCH"):
        if n_in:
            val = int(n_in[-2:]); r_v = 0
            if st.session_state.history:
                res_temp = analyze_v74(st.session_state.history, st.session_state.last_n)
                if res_temp:
...
# --- 6. HIỂN THỊ ---
if st.session_state.history:
    results = analyze_v74(st.session_state.history, st.session_state.last_n)
    if results:
        ...
        tab1, tab2 = st.tabs(["🎯 DÀN TINH ANH", "📊 NHẬT KÝ ĐẦY ĐỦ"])
        with tab1:
            cols = st.columns(8)
            for i, r in enumerate(results):
                with cols[i]:
                    st.markdown(f"<div class='bit-header'>{r['l']}</div>", unsafe_allow_html=True)
                    st.markdown(f"<div class='bit-card'><b>Hội tụ: {int(r['f']*100)}%</b><br><span class='{'sample-ok' if r['c4']>=10 else 'sample-low'}'>Mẫu 4K: {r['c4']}</span></div>", unsafe_allow_html=True)
            st.divider()
    ...
    with tab2:
        # BẢNG NHẬT KÝ ĐẦY ĐỦ CÁC THÔNG SỐ
        disp = []
        for h in sorted(st.session_state.history, key=lambda x: x['Kỳ'], reverse=True):
            b = get_8bit(h["Số"])
            row = {
                "Kỳ": h["Kỳ"], "Số": h["Số"], "Rank": h["Rank"],
                "Đ.CL": "Lẻ" if b[0] else "Chẵn",
                "Đu.CL": "Lẻ" if b[1] else "Chẵn",
                "T.CL": "Lẻ" if b[2] else "Chẵn",
                "Đ.TB": "To" if b[3] else "Bé",
                "Đu.TB": "To" if b[4] else "Bé",
                "T.TB": "To" if b[5] else "Bé",
                "Hệ": "Thuận" if b[6] else "K.Phải",
                "Hiệu": "To" if b[7] else "Bé"
            }
            disp.append(row)
