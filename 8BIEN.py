...
def analyze_v85(history, last_n):
    ...
    # CẤU HÌNH ĐỐI TRỌNG V8.5
    LIMIT_TIEN_TUYEN = 11  # Nhịp tấn công gắt
    LIMIT_TRUNG_QUAN = 22  # Nhịp ổn định
    LIMIT_HAU_PHUONG = 66  # Nhịp đối trọng (Mỏ neo)
    MIN_SAMPLE = 3

    for i in range(8):
        # 1. TIỀN TUYẾN (4K - Trọng số cao nhất)
        ...
        # 2. TRUNG QUÂN (3K - Nhịp trung hạn)
        ...
        # 3. HẬU PHƯƠNG (22K - Đối trọng dài hạn 66 mẫu)
        pm_pair = []
        for j in range(8):
            if i == j: continue
        ...
        # CÔNG THỨC CÂN BẰNG (EQUILIBRIUM)
        # Tấn công (p4+p_mom): 60% | Ổn định (p3): 20% | Đối trọng (p_base): 20%
        ...
        results.append({
            ...
            "c4": len(m4_lim), "p4": p4, 
            "p_mom": p_mom, 
            "c_base": len(pm_pair), "p_base": p_base, 
            "f": f_prob
        })
    return results
...
if c3.button("🚀 PHÂN TÍCH"):
    if n_in:
        ...
        if st.session_state.history:
            res_temp = analyze_v85(st.session_state.history, st.session_state.last_n)
            if res_temp:
...
# --- 6. HIỂN THỊ ---
if st.session_state.history:
    results = analyze_v85(st.session_state.history, st.session_state.last_n)
    if results:
        tab1, tab2 = st.tabs(["🎯 PHÂN TÍCH ĐỐI TRỌNG", "📊 NHẬT KÝ ĐẦY ĐỦ"])
        with tab1:
