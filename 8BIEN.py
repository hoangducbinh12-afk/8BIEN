...
# --- 1. GIAO DIỆN CHUẨN V6.9 ---
st.set_page_config(page_title="8-BIT QUANTUM V6.9", layout="wide")
st.markdown("""
    <style>
    ...
    .sample-low { color: #94a3b8; font-style: italic; font-size: 0.65rem; }
    .sample-ok { color: #10b981; font-weight: bold; font-size: 0.65rem; }
    </style>
...
def analyze_full_v69(history, last_n):
    all_bits = np.array([get_8bit(h["Số"]) for h in history])
    curr_bits = np.array(get_8bit(last_n))
    results = []
    
    MIN_SAMPLE = 10 # CHỐT CHẶN 10 LẦN CỦA MÀY

    for i in range(8):
        s4 = "".join(map(str, all_bits[-4:, i].astype(int)))
        
        # 1. Quét lịch sử tìm mẫu 4 kỳ lặp lại
        matches_4k = []
        for k in range(len(all_bits) - 5):
            if np.array_equal(all_bits[k:k+4, i], all_bits[-4:, i]):
                matches_4k.append(all_bits[k+4, i])
        
        count_4k = len(matches_4k)
        # Nếu mẫu lặp lại < 10 lần, p4 = 0.5 (Không cho phép tác động)
        p4 = np.mean(matches_4k) if count_4k >= MIN_SAMPLE else 0.5
        
        # 2. Momentum 10 kỳ (Luôn đủ mẫu vì quét 10 kỳ gần nhất)
        p_mom = np.mean(all_bits[-10:, i])
        
        # 3. Cặp 22 kỳ
        p_pair = 0.5; count_pair = 0
        if len(history) >= 22:
            ...
            for j in range(8):
                ...
                if m: pm.extend(m)
            count_pair = len(pm)
            if count_pair >= MIN_SAMPLE: p_pair = np.mean(pm)

        f_prob = (p4 * 0.45) + (p_mom * 0.30) + (p_pair * 0.25)
        results.append({
            "l": BIT_LABELS[i], "s4": s4, "c4": count_4k, "p4": p4, 
            "p_mom": p_mom, "c_pair": count_pair, "p_pair": p_pair, "f": f_prob
        })
    return results
...
st.title("🛡️ QUANTUM MASTER V6.9 (THE VALIDATOR)")
with st.container():
    ...
    if c2.button("🚀 PHÂN TÍCH"):
        if n_in:
            ...
            if st.session_state.history:
                res = analyze_full_v69(st.session_state.history, st.session_state.last_n)
                p = [r["f"] for r in res]
...
if st.session_state.history:
    results = analyze_full_v69(st.session_state.history, st.session_state.last_n)
    ...
    tab1, tab2 = st.tabs(["🎯 DÀN & XÁC MINH MẪU", "📊 NHẬT KÝ"])
    with tab1:
        cols = st.columns(8)
        for i, r in enumerate(results):
            with cols[i]:
                s4_color = "sample-ok" if r['c4'] >= 10 else "sample-low"
                pair_color = "sample-ok" if r['c_pair'] >= 10 else "sample-low"
                st.markdown(f"""
                <div class='bit-header'>{r['l']}</div>
                <div class='bit-card'><b>4K ({r['s4']})</b><br><span class='{s4_color}'>Mẫu: {r['c4']} lần</span></div>
                <div class='bit-card'><b>10 Kỳ:</b> {int(r['p_mom']*100)}%</div>
                <div class='bit-card'><b>22 Kỳ</b><br><span class='{pair_color}'>Mẫu: {r['c_pair']} lần</span></div>
                <div class='bit-card' style='background:#e0f2fe; border: 1px solid #000080'><b>Hội tụ: {int(r['f']*100)}%</b></div>
                """, unsafe_allow_html=True)
        st.divider()
        ca, cb = st.columns([2, 1])
        num_quan = cb.number_input("Số quân lấy dàn:", value=50, step=1)
...
with st.sidebar:
    ...
    if up:
        ...
        st.rerun()
    if st.session_state.history:
        st.download_button("💾 BACKUP", json.dumps({"history": st.session_state.history}), f"8bit_v6.9.json")
